...
**Goal:** Rewrite `README.md` as a hub page, replacing the outdated "Running OCAP" section with a concise "Getting Started" section that links to the new docs.

**Architecture:** Single file replacement. The new README preserves all existing prose verbatim except the "Running OCAP" section, which is replaced with a "Getting Started" section containing a 3-step install summary, a config callout, and the verbatim Terrains and Usage content.
...
- [ ] **Step 1: Read the current README to confirm its content**

```bash
cat -n /home/jacob/OCAP/README.md
```

Verify the file exists and note the line numbers of the "Running OCAP" section (starts around line 36, ends around line 123).

- [ ] **Step 2: Write the new README.md**

Overwrite `/home/jacob/OCAP/README.md` with exactly the following content:
...
if (isServer) then {
	...
	[east, "OCAP has achieved all of their objectives!"] call ocap_fnc_exportData;
...
- [ ] **Step 3: Verify the key changes are present**

```bash
grep -n "Getting Started\|Installation Guide\|Configuration Reference\|ocap_recorder.cfg.json\|setting.json\|Running OCAP\|userconfig\|OcapReplaySaver2" /home/jacob/OCAP/README.md
```
...
```bash
git -C /home/jacob/OCAP add README.md
git -C /home/jacob/OCAP commit -m "docs: rewrite README as hub page with links to installation and configuration guides"
```
