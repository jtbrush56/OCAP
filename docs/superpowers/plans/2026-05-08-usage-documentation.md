# Usage Documentation Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Write `docs/usage.md` — a complete OCAP usage guide covering mission integration (SQF/CBA API) and the web playback UI.

**Architecture:** Single Markdown file. Part 1 covers the server-side SQF/CBA API for mission designers. Part 2 covers the web playback UI for players and viewers. Written in two tasks so each part can be committed independently.

**Tech Stack:** Markdown only. No code changes. Sources: `addon/addons/recorder/fnc_addEventMission.sqf`, `fnc_exportData.sqf`, `fnc_handleCustomEvent.sqf`, `fnc_startRecording.sqf`, `fnc_adminUIcontrol.sqf`, `web/ui/src/pages/recording-playback/shortcuts.ts`.

---

### Task 1: Write `docs/usage.md` — Part 1: Mission Integration

**Files:**
- Create: `docs/usage.md`

- [ ] **Step 1: Create `docs/usage.md` with Part 1**

Use the Write tool to create `/home/jacob/OCAP/docs/usage.md` with the following content. The content uses triple-backtick code fences inside it, so copy it exactly:
...
- [ ] **Step 2: Verify Part 1 key content is present**

```bash
grep -n "Auto-Start\|ocap_fnc_exportData\|OCAP_record\|OCAP_pause\|OCAP_customEvent\|OCAP_counterInit\|OCAP_setFocusStart\|Admin Controls" /home/jacob/OCAP/docs/usage.md
```

Expected: all eight terms appear in the output.

- [ ] **Step 3: Commit**

```bash
git -C /home/jacob/OCAP add docs/usage.md
git -C /home/jacob/OCAP commit -m "docs: add usage guide Part 1 — mission integration"
```

---

### Task 2: Append Part 2 (Playback UI), verify, and commit

**Files:**
- Modify: `docs/usage.md` (append)

- [ ] **Step 1: Append Part 2 to `docs/usage.md`**

Use the Edit tool to append the following to `/home/jacob/OCAP/docs/usage.md`. Find the last line of the file (ending with "regardless of admin status.") and replace it with that line plus the new content below:
...
- [ ] **Step 2: Verify Part 2 key content is present**

```bash
grep -n "Finding a Recording\|Keyboard Shortcuts\|Following a Unit\|Side Panel\|Focus Mode\|Counter Display" /home/jacob/OCAP/docs/usage.md
```

Expected: all six terms appear in the output.

- [ ] **Step 3: Verify no placeholder text**

```bash
grep -in "TBD\|TODO\|placeholder\|fill in\|implement later" /home/jacob/OCAP/docs/usage.md
```
...
```bash
git -C /home/jacob/OCAP add docs/usage.md
git -C /home/jacob/OCAP commit -m "docs: add usage guide Part 2 — playback UI"
```
